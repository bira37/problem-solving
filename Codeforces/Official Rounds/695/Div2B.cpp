...
void solution(int testcase) {
  ...
  auto chk = [&](int x) {
    if (v[x] > max(v[x + 1], v[x - 1]) || v[x] < min(v[x + 1], v[x - 1]))
      return 1;
    else
      return 0;
  };
  ...
  for (int i = 0; i < n; i++) {
    // remove originals
    if (i > 0 && i < n - 1) cur -= chk(i);
    if (i + 1 > 0 && i + 1 < n - 1) cur -= chk(i + 1);
    if (i - 1 > 0 && i - 1 < n - 1) cur -= chk(i - 1);
    ...
    if (i - 1 >= 0) {
      ...
      int more = 0;
      if (i > 0 && i < n - 1) more += chk(i);
      if (i + 1 > 0 && i + 1 < n - 1) more += chk(i + 1);
      if (i - 1 > 0 && i - 1 < n - 1) more += chk(i - 1);

      ans = min(ans, cur + more);
    }
