...
bool check(string s, string t){
  int cu1 = ((s[0] - '0') + (s[1] - '0') + (s[2] - '0'));
  int cu2 = ((t[3] - '0') + (t[4] - '0') + (t[5] - '0'));
  if(cu1 == cu2) return true;
  else return false;
} 
...
int32_t main(){
  ...
  for(int i=1; i<=n; i++){
    for(int j=1; j<=m; j++){
      if(check(a[i], b[j]) || check(b[j], a[i])){
  ...
  for(int i=1; i<=n; i++){
    for(int j=1; j<=m; j++){
      ...
      for(Dinic::FlowEdge e : dinic.adj[i]){
        if(e.v == want && e.c == 0) found = true;
      }
      if(found){
        ans.pb(make_pair(a[i], b[j]));
      }
    }
  }
  cout << ans.size() << endl;
  for(auto tp : ans){
    if(check(tp.ff, tp.ss)) cout << "AT " << tp.ff << " " << tp.ss << endl;
    else cout << "TA " << tp.ss << " " << tp.ff << endl;
  }
}
