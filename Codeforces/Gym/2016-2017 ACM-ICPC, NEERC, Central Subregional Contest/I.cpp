...
int32_t main(){
  ...
  for(int i = 0; i < n; i++) {
    // point to point
      ...
      ans = min(ans, (town[i]-town[j]).size());
  }
  ...
  for(int i = 0; i < n; i++) {
    // point to segment
     ...
     // cout << i << " " << j << " " << (j+1)%n << endl;
      if(n-(k-2)+1 != m) break;
      Geo2D::Line s(town[j], town[(j+1)%n]);
      ans = min(ans, Geo2D::distancePointSegment(town[i], s));
     // cout << ans << endl;
  }
  ...
  for(int i = 0; i < n; i++) {
    // segment to segment
      ...
      j%=n;
      if(n-(k-2)+2 != m) break;
      Geo2D::Line s1(town[i], town[(i+1)%n]);
      Geo2D::Line s2(town[j], town[(j+1)%n]);
      ans = min(ans, Geo2D::distanceSegmentSegment(s1, s2));
  }
