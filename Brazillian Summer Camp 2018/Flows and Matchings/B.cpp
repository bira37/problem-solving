...
void solution() {
  ...
  for (ii val : s) {
    ...
    int y = val.ss;
    if ((x + y) % 2 == 0)
      dinic.add_to_src(id[ii(x, y)], 1);
    else
      dinic.add_to_snk(id[ii(x, y)], 1);
    int dx[] = {0, 1, 0, -1};
    int dy[] = {-1, 0, 1, 0};
    if ((x + y) % 2) continue;
    for (int w = 0; w < 4; w++) {
      int nx = x + dx[w];
      int ny = y + dy[w];
      if (s.count(ii(nx, ny))) dinic.add_edge(id[ii(x, y)], id[ii(nx, ny)], 1);
    }
  }
  dinic.calculate();
  if (p % 2 || (p / 2) != dinic.max_flow) {
    cout << "No" << endl;
    return;
  }
  cout << "Yes" << endl;
  ...
  for (int i = 1; i <= n * n; i++) {
    ii coord = rev[i];
    if ((coord.ff + coord.ss) % 2) continue;
    for (Dinic::FlowEdge e : dinic.adj[i]) {
      if (e.v == dinic.src) continue;
      if (e.c == 0) {
        edge_cover.pb(ii(i, e.v));
      }
    }
